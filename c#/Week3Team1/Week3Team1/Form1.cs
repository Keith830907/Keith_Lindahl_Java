    ...
    public partial class Form1 : Form
    {
        ...
        private void ReadSteps(XDocument xDoc)
        {
            ...
            foreach (XElement direction in directions)
            {
                ...
                XElement eachDirection = direction;
                string parsedDirection = Regex.Replace(eachDirection.Value, @"<.*?>", "");
                stepsList.Add(parsedDirection);
            }
        ...
        private void CreateStepsTable()
        {
            ...
            foreach (var step in stepParts)
            {
                ...
                row["Distance"] = (Int32.Parse(step.stepAndDistance.distance) / 1609).ToString() + "mi";
                row["Time"] = (Int32.Parse(step.duration) / 60).ToString() + "mins";
